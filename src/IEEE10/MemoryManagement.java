...
public class MemoryManagement {
    ...
    private static void solve() throws IOException {
        ...
        while (T-- > 0) {
            ...
            int FIFO = 0;
            Map<Integer, Integer> prrority = new HashMap<>();
            Queue<Integer> queue = new PriorityQueue<>(Comparator.comparingInt(prrority::get));
            int cur = 0;
            for (int i = 0; i < n; i++) {
                if (queue.contains(a[i])) {
                    continue;
                }
                prrority.put(a[i], cur++);
                queue.offer(a[i]);
                if (queue.size() > p) {
                    queue.poll();
                    FIFO++;
            ...
            if (FIFO <= LRU) {
                pw.print("no " + FIFO + " " + LRU);
            } else {
                pw.print("yes " + FIFO + " " + LRU);
            }
